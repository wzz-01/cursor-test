...
COALESCE(dim_cust.region_name, c.大区, J.大区) AS c_daqu, COALESCE(dim_cust.province_region_name, c.省区 ,J.省区 ) AS c_shengqu,
COALESCE(dim_cust.sales_group_name, c.销售组, vs.store_dept_name) AS c_xiaoshouzu, c.直营经销 AS c_qudao,
...
LEFT JOIN (
    -- 子查询：销售组兜底，每组一行，避免 DISTINCT 一对多撑行
